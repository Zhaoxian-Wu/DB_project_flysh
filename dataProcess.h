...
void GaussionMatrix(string sourceMatrixName, size_t m, size_t dim);

// build a fly projection matrix using a given probability p
void FlyMatrix(string sourceMatrixName, size_t m, size_t dim, float p);
...
void randomMatrix(string sourceMatrixName, size_t k);

// 7.基于果蝇投影矩阵哈希后，应用了WTA机制的数据。
void WTAMatrix(string sourceMatrixName, size_t k);

// 8.基于果蝇投影矩阵哈希后，应用了binary机制的数据。
void binaryMatrix(string sourceMatrixName, size_t k);